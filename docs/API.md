...
- **Android: rowid overflow bug** — DuckDB's FTS extension may fail on Android with "Information loss on integer cast" due to internal `rowid` values exceeding `int64` range during index creation. The FTS indexing script (`fts_indexing.cpp`) uses `SELECT rowid AS docid FROM table`, and on Android the `rowid` for in-memory tables can produce unsigned 64-bit values that overflow signed `int64`. This is an upstream DuckDB bug — not yet filed, report at [duckdb/duckdb-fts](https://github.com/duckdb/duckdb-fts/issues). FTS indexing works reliably on iOS. The `stem()` function works on both platforms.
